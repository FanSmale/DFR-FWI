...
# mse
def criterion_l2(pred, gt):
    l2loss = nn.MSELoss()
    loss_l2 = l2loss(pred, gt)
    return loss_l2
...
def criterion_pixel_grad(pred, target, alpha=1.0, beta=0.3):
    l1 = F.l1_loss(pred, target)
    l2 = F.mse_loss(pred, target)
    grad_loss = gradient_loss(pred, target)

    total_loss = alpha * (l1 + l2) + beta * grad_loss
    return total_loss, l1, l2, grad_loss
