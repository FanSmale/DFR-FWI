...
from innovation.BRA import BiLevelRoutingAttention
...
class Biformer(nn.Module):
    ...
    def forward(self, x):
        """
        x: NCHW tensor → (B, C, H, W)
        return: NCHW tensor → (B, C, H, W)
        """
        B, C, H, W = x.shape
        # assert (H, W) == self.input_resolution, f"Input resolution {H, W} != expected {self.input_resolution}"

        shortcut = x

        # Add pos_embed (DWConv) in NCHW
        x = x + self.pos_embed(x)  # (B, C, H, W)

        # Convert to NHWC for attention & norm
        x = x.permute(0, 2, 3, 1)  # (B, H, W, C)

        # LayerNorm in NHWC
        x = self.norm1(x)  # operates on last dim (C)

        # Attention (expects NHWC, returns NHWC)
        if self.pre_norm:
            attn_out = self.attn(x)  # (B, H, W, C)
        ...
        # MLP (in NHWC)
        x_mlp = self.norm2(x) if self.pre_norm else x
