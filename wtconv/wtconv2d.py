...
class WTConv2d(nn.Module):
    """
    在保持输入输出通道数一致（in_channels == out_channels）的前提下，
    通过小波域中的特征增强来提升对地震数据中多尺度结构（尤其是低频全局信息）的建模能力，
    并最终与原始空间域卷积结果融合，形成更强的表示。
    """
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=1, bias=True, wt_levels=1, wt_type='db1'):
        """

        wt_levels：小波分解的层数（例如 1 表示只做一次 DWT，得到 LL/LH/HL/HH 四个子带）。
        wt_type='db1'：使用 Daubechies 小波（db1 即 Haar 小波），也可换为 db2, sym4 等。
        """
